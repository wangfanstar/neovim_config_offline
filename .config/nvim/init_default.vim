...
Plug '~/.local/share/nvim/plug/nvim-tree.lua'
...
" 打开搜索结果时调用rg进行搜索
let $FZF_DEFAULT_OPTS = '--preview-window "right:60%:hidden"'
...
function! ToggleBufList()
  let buflist_win = bufwinnr('__BufList__')
  if buflist_win != -1
    execute buflist_win . 'wincmd w'
    bwipeout
    return
  endif
  topleft 10new __BufList__
  ...
  for b in bufs
    ...
    call add(lines, printf('%3d  %s%s', b, name, modified))
  endfor
  call setline(1, lines)
  setlocal nomodifiable
  " 按 Enter 跳转，按 q 或 Ctrl+E 关闭
  nnoremap <buffer> <CR> :call <SID>BufListJump()<CR>
  nnoremap <buffer> q :bwipeout<CR>
  nnoremap <buffer> <C-e> :bwipeout<CR>
endfunction

function! s:BufListJump()
  let line = getline('.')
  let bufnum = str2nr(matchstr(line, '^\s*\zs\d\+'))
  bwipeout
  if bufnum > 0
...
nnoremap <C-e> :call ToggleBufList()<CR>
...
-- 加载 fzf 拓展（libfzf.so 不存在时静默跳过，需在插件目录执行 make）
local ok, err = pcall(require('telescope').load_extension, 'fzf')
if not ok then
  vim.notify('telescope-fzf-native 未编译，请在插件目录执行 make\n' .. err, vim.log.levels.WARN)
end
EOF



" treesitter 已禁用（离线环境无法安装 parser）
" 如需启用，先执行 :TSInstall c 后取消下方注释
" lua << EOF
...
" 基于 ctags tags 文件高亮自定义函数和类型
function! LoadTagsHighlight()
  if !filereadable('tags')
    return
  endif
  " 限制读取行数，避免大 tags 文件阻塞
  let max_lines = 5000
  let lines = readfile('tags', '', max_lines)
  let types = []
  let funcs = []
  let macros = []
  for line in lines
    ...
    elseif kind ==# 'f'
      call add(funcs, name)
    elseif kind ==# 'd'
      call add(macros, name)
    endif
  endfor
  highlight cCustomFunction ctermfg=lightblue guifg=#83a598
  highlight cCustomMacro    ctermfg=yellow    guifg=#fabd2f
  if !empty(types)
    execute 'syntax keyword cUserType ' . join(uniq(sort(types)))
    highlight default link cUserType cType
  endif
  if !empty(funcs)
    execute 'syntax keyword cUserFunction ' . join(uniq(sort(funcs)))
    highlight default link cUserFunction cCustomFunction
  endif
  if !empty(macros)
    execute 'syntax keyword cUserMacro ' . join(uniq(sort(macros)))
    highlight default link cUserMacro cCustomMacro
  endif
endfunction

" 打开 C/C++ 文件时自动加载
augroup TagsHighlight
  autocmd!
  autocmd FileType c,cpp call LoadTagsHighlight()
  autocmd FileType c,cpp hi link cFormatError NONE
augroup END
